...
path = 'C:\Users\Ильгиз\Documents\Документы_Ильгиз\DTU\Research\Collaboration with Andreas\neural_network\Last_dataset_from_Timon_18_Nov_2020\';
...
%     [err,~,pfSol] = getLosses(milpRes, mpcOpt);
    [err,PFstatus,loss,pfSol] = getLosses(milpRes, mpcOpt,NGenIncSlack);
    fprintf('AC Real Loss: %2.3f\n',loss.ac_real);
    fprintf('MILP Loss: %2.3f\n',loss.milp_real);
...
acopfRes = analyzeResults(mpcOrigin,mpcOpt,obj,setpoints,genRange,NN_input, NN_output,vRange,NGenExcSlack);
...
function varargout = analyzeResults(mpc,mpOption,obj,setpoints,genRange,NN_input, NN_output,vRange,NGenExcSlack)
chr = fprintf('|  MILP objective value:     <strong>%3.2f $/MW </strong> |\n',value(obj));
chr = chr -17;
fprintf(strcat('|', repmat('-',1,chr-3),'|\n'))

genList = util.GetGenList(mpc);
[classification,classDetails,dampingR,pfSuccess] = util.classifyCase_PGVG(mpc,setpoints,mpOption,NGenExcSlack);
...
setpointsACOPF = [results.var.val.Pg(2:end) * results.baseMVA; results.var.val.Vm(1:NGenExcSlack+1)];
[classification,classDetails,dampingR,pfSuccess] = util.classifyCase_PGVG(mpc,setpointsACOPF,mpOption,NGenExcSlack);
...
compare_feas_dist(setpoints, setpointsACOPF,genRange,NN_input, NN_output,chr,vRange,NGenExcSlack)
if nargout ==1 
   varargout{1} = results;
end
end
...
function compare_feas_dist(setpoints, setpointsACOPF,genRange,NN_input,NN_output,chr,vRange,NGenExcSlack)
...
allStablePoints = horzcat(NN_input(logical(NN_output(:,1)),1:NGenExcSlack).*genRange', NN_input(logical(NN_output(:,1)),NGenExcSlack+1:end).*vRange');
