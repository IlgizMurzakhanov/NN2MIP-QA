...
NStopIter = 50; % number of stopping iterations in err loop
...
for k = 1:nEps:nPoints
    ...
    vGeneration_fixed = [1.06;1.06;1.06;0.94;0.94]; 
...
%             + sum(-l(2:NGenIncSlack).*(u_PG.*optvar2pu - pGeneration(GenGenInd)))... 
%             + sum(-l_v(2:NGenIncSlack+1).*(Vmin + u_VG.*vRange - vGeneration)) == 0):'lossy power balance'];%#ok
            ...
            diagnostics.info
%             milpRes.setpoints  = [value(Pslack)*mpc.baseMVA; value(u_PG).*genRange; Vmin + value(u_VG).*vRange]
...
%         milpRes.diag = diagnostics;
        ...
        vsetpoints(k+j,:) = value(u_VG);%setpoints(:);
...
save(fileName,'setpoints','pslack','spaceGrid','price','stability','gsetpoints','vsetpoints','dampingRatio','resultStruct','epsArray')
%%%% saved variables
